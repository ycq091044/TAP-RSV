...
class LitModel(pl.LightningModule):
    def __init__(self, args, disease_tensor_emb):
        ...
        self.model = Transformer(hidden=32, test_length=args.test_length)
...
def prepare_dataloader(args):
    ...
    for  i in range(2334):
        ...
        test_cty.append(i)
        test_Y.append(bron[i, test_start:test_start+4, 11]) # 11 means the RSV cases
...
def supervised(args):
    ...
    version = f"Transformer-{args.val_start}-{args.test_start}-len-{args.test_length}-{version_id}"
    ...
    # dump
    with open("../logs/results/transformer.log", "a") as f:
