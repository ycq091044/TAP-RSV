...
from utils import load_dataset, RSVLoader_STAN, collate_fn_stan, mean_absolute_percentage_error, ccc, r_squared, pcc
...
class LitModel(pl.LightningModule):
    def __init__(self, args, disease_tensor_emb):
        ...
        self.model = STAN(gat_hidden=32, hidden=32, static_dim=32, test_length=args.test_length)

    def training_step(self, batch, batch_idx):
        county, covid, distance, mob, vac, hos, claim, county_tensor_emb, X_mtx, mask, month, Y, adj = batch
        Y = torch.log(Y+1)
        prediction = self.model(county, covid, vac, hos, claim, county_tensor_emb, X_mtx, month, mask, adj)
...
def prepare_dataloader(args):
    ...
    batch_size = 256
    train_loader = torch.utils.data.DataLoader(RSVLoader_STAN(county, covid, distance, mob, vac, hos, claim, county_tensor_emb, train_X, train_cty, train_month, train_Y), 
                    batch_size=batch_size, shuffle=True, collate_fn=collate_fn_stan, num_workers=32, drop_last=True)
    val_loader = torch.utils.data.DataLoader(RSVLoader_STAN(county, covid, distance, mob, vac, hos, claim, county_tensor_emb, val_X, val_cty, val_month, val_Y), 
                    batch_size=batch_size, shuffle=False, collate_fn=collate_fn_stan, num_workers=32)
    test_loader = torch.utils.data.DataLoader(RSVLoader_STAN(county, covid, distance, mob, vac, hos, claim, county_tensor_emb, test_X, test_cty, test_month, test_Y), 
                    batch_size=batch_size, shuffle=False, collate_fn=collate_fn_stan, num_workers=32)
...
def supervised(args):
    ...
    version = f"STAN-{args.val_start}-{args.test_start}-len-{args.test_length}-{version_id}"
    ...
    # dump
    with open("../logs/results/stan.log", "a") as f:
...
if __name__ == "__main__":
    ...
    # {'mse': 29.97538, 'mae': 2.3563318, 'mape': 1.0922763, 'ccc': 0.6911959593471351, 'rsquare': 0.4470846332868522, 'pcc': 0.6911959438564704}
    # model performance:
    # test_ccc_loss         0.7719015952992792
    # test_mae_loss         2.1681456565856934
    # test_mape_loss         1.0584971904754639
    # test_mse_loss         25.980022430419922
    # test_pcc_loss         0.7719015530730702
    # test_rsquare_loss       0.5199967701313469
    supervised(args)
