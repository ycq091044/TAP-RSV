...
class LitModel(pl.LightningModule):
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        for idx, out in enumerate(test_step_outputs):
            predicted.append(out[0])
            gt.append(out[1])
        y_pred = np.concatenate(predicted, axis=0)
        y_true = np.concatenate(gt, axis=0)
        
        idx = len(os.listdir("../logs/results-exp4")) // 2 + 1
        with open(f"../logs/results-exp4/y_pred_{idx}.npy", "wb") as f:
            np.save(f, y_pred)
        with open(f"../logs/results-exp4/y_true_{idx}.npy", "wb") as f:
            np.save(f, y_true)
...
def supervised(args):
    ...
    version_id = len(os.listdir(os.path.join(root, "Evaluation"))) + 1
    ...
    # remember, to test the model, we must use only one GPU, otherwise the county order will be messed up
    paths = ["../logs/RSV-analysis-exp1/TAPRSV-2022-09-17-2022-10-15-13/checkpoints/epoch=12-step=7436.ckpt",
             "../logs/RSV-analysis-exp1/TAPRSV-2022-09-17-2022-10-15-14/checkpoints/epoch=7-step=4576.ckpt",
             "../logs/RSV-analysis-exp1/TAPRSV-2022-09-17-2022-10-15-16/checkpoints/epoch=10-step=6292.ckpt",
             "../logs/RSV-analysis-exp1/TAPRSV-2022-09-17-2022-10-15-18/checkpoints/epoch=8-step=5148.ckpt",
             "../logs/RSV-analysis-exp1/TAPRSV-2022-09-17-2022-10-15-19/checkpoints/epoch=10-step=6292.ckpt",
    ]
    for path in paths:
        trainer.test(model=model, ckpt_path=path, dataloaders=test_loader)
